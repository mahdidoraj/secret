...
do
local alireza_PT = 159887854 --put your id here(BOT OWNER ID)
...
local function res_user_callback(extra, success, result) -- /info <username> function
 ...
 local value = redis:hget(hash, result.id)
    if not value then
  if result.id == tonumber(mehrpouya) then
    send_document(get_receiver(msg), "/root/files/me/sudo.webp", ok_cb, false)
    text = text..'Who : Executive Admin \n'
   elseif is_sudo(result.id) then
    send_document(get_receiver(msg), "/root/files/me/sudo.webp", ok_cb, false)
    text = text..'Who : Sudo \n'
   elseif is_admin2(result.id) then
    send_document(get_receiver(msg), "/root/files/me/admin.webp", ok_cb, false)
    text = text..'Who : Admin \n'
   elseif is_owner2(result.id, extra.chat2) then
    send_document(get_receiver(msg), "/root/files/me/leader.webp", ok_cb, false)
    text = text..'Who : Leader \n'
   elseif is_momod2(result.id, extra.chat2) then
     send_document(get_receiver(msg), "/root/files/me/moderator.webp", ok_cb, false)
     text = text..'Who : Moderator \n'
      else
     send_document(get_receiver(msg), "/root/files/me/member.webp", ok_cb, false)
     text = text..'Who : Member \n'
  end
   else
   text = text..'Who : '..value..'\n'
  end
...
local function action_by_reply(extra, success, result)-- (reply) /info  function
 ...
 local hash = 'rank:'..result.to.id..':variables'
  local value = redis:hget(hash, result.from.id)
   if not value then
      if result.from.id == tonumber(mehrpouya) then
            send_document(get_receiver(msg), "/root/files/me/sudo.webp", ok_cb, false)
         text = text..'Who : Executive Admin \n'
...
local function run(msg, matches)
 if matches[1]:lower() == 'setwho' then
 ...
 if hash then
   ...
   if not value then
  if msg.from.id == tonumber(mehrpouya) then
   send_document(get_receiver(msg), "/root/files/me/sudo.webp", ok_cb, false)
   text = text..'Who : Executive Admin \n'
 ...
 text = text..'\n\n👁‍🗨Security Team'
  ...
  if matches[1]:lower() == 'setwho' and matches[2] then
